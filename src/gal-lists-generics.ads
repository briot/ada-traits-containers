...
pragma Ada_2012;
with GAL.Cursors;
with GAL.Lists.Storage;
with GAL.Lists.Impl;
with GAL.Properties;

generic
   with package Storage is new GAL.Lists.Storage.Traits (<>);
   --  Describes how the nodes of the list are stored.

package GAL.Lists.Generics with SPARK_Mode is
   ...
   package Impl is new GAL.Lists.Impl (Storage);
   ...
   package Maps is
      ...
      package Constant_Returned is new GAL.Properties.Read_Only_Maps
        (Cursors.Forward.Container, Cursors.Forward.Cursor,
         Storage.Elements.Constant_Returned, GAL.Lists.Generics.Element);
   end Maps;
